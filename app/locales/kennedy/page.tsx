...
export default function KennedyPage() {
  const [data, setData]           = useState<KennedyData[]>([]);
  ...
  useEffect(() => {
    fetch('/api/locales/kennedy')
      .then(r => r.json())
      .then((d: KennedyData[]) => {
        const sorted = [...d].sort((a, b) => parseMes(a.mes) - parseMes(b.mes));
        setData(sorted);
  ...
  const mesesVisibles = useMemo(() => {
    ...
    if (idx === -1) return [];
    return mesesDisponibles.slice(Math.max(0, idx - 2), idx + 1);
